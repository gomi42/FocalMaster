// https://github.com/gheeres/PDFSharp.Extensions/tree/master
...
namespace PdfSharp.Pdf
{
    ...
    public static class PdfDictionaryExtensions
    {
        ...
        private static Bitmap ImageFromDCTDecode(PdfDictionary dictionary)
        {
            // DCTDecode a lossy filter based on the JPEG standard
            // We can just load directly from the stream.
            using (MemoryStream stream = new MemoryStream(dictionary.Stream.Value))
            {
        ...
        /// <param name="dictionary">The dictionary to extract the object from.</param>
        /// <returns>The image retrieve from the dictionary. If not found or an invalid image, then null is returned.</returns>
        private static Bitmap ImageFromCCITTFaxDecode(PdfDictionary dictionary)
        {
        ...
        /// <param name="dictionary">The dictionary to extract the object from.</param>
        /// <returns>The image retrieve from the dictionary. If not found or an invalid image, then null is returned.</returns>
        private static Bitmap ImageFromFlateDecode(PdfDictionary dictionary)
        {
            PdfDictionaryImageMetaData imageData = new PdfDictionaryImageMetaData(dictionary);
        ...
        public static Bitmap ToImage(this PdfDictionary dictionary)
        {
            if (!IsImage(dictionary))
                return (null);
            ...
            Func<PdfDictionary, Bitmap> noAction = (d) => null;
            var map = new Dictionary<string, Func<PdfDictionary, Bitmap>>()
            {
            ...
            if (element.IsName())
                filter = ((PdfName)element).Value;
            else
            if (element.IsArray())
                return (ToImage(ProcessFilters(dictionary)));
        ...
        abstract class PdfIndexedColorSpace : PdfColorSpace
        {
            ...
            {
                if (array == null)
                    throw new ArgumentNullException("array", "The indexed color array was null.");
            ...
            {
                if (reference == null)
                    throw new ArgumentNullException("reference", "The reference index was null.");
        ...
        class PdfIndexedRGBColorSpace : PdfIndexedColorSpace
        {
            ...
            {
                if (colorSpace == null)
                    throw new ArgumentNullException("colorSpace", "The colorspace must be specified when creating a PdfIndexedRGBColorSpace.");
        ...
        class PdfGrayColorSpace : PdfRGBColorSpace
        {
            ...
            {
                if (bitsPerPixel > 8)
                    throw new ArgumentException("bitsPerPixel", "Can't support grayscale image with more than 8 bits per pixel.");
